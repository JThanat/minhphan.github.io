...
The difference between LHS and RHS of Eq.(4.3) is the KL divergence between the variational $$q(\theta, \mathbf{z}|\gamma, \phi)$$ and true posterior probability $$p(\theta, \mathbf{z}, \mathbf{w}|\alpha,\beta)$$. Let RHS of Eq.(4.3) $$\delequal$$ $$L(\gamma,\phi;\alpha,\beta)$$ we have
...
% $$\text{log}p(\mathbf{w|z},\beta) = \text{log}\beta_i^{z_{n,i}.w_{n,j}} = {z_{n,i}.w_{n,j}}\text{log}\theta_i$$.
