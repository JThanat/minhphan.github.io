...
123 where $$\phi_i$$ the probability over the vocabulary generated by latent topic $$i$$,
...
Dir($$\alpha$$) is a draw from a uniform Dirichlet distribution with parameter $$\alpha$$ and multi(.) is a Multinomial distribution. <!-- Note that $$w_{d,n} \sim$$ $$p(w_{d,n}|z_{d,n},\beta_i)$$ = $$\beta_{z_{d,n},i}$$ = $$\beta_{z_{d,n}}$$, -->
